...
<Update label="2026-05-19" tags={["Feature","Update"]} description="v5.4.0">
  ...
  - **Expanded custom hit & kill sounds** — Large new collection of satisfying sounds. Instantly preview any sound directly from the menu when changing your selection.
  - **Health priority targeting** — Aim assist now prioritizes lower-health targets for smarter focus fire.
  ...
  - **Unified bomb radar icons** — Planted, dropped, and carrier bombs now use consistent, clear icons with a thin black border for better readability.
  ...
  - **Removed TriggerBot, AimLock & AutoShot** — These legacy features have been fully removed to simplify the cheat and improve overall stability.
  - **ESP overhaul for stability** — Completely removed the previous TraceShape visibility system (which could cause instability). Now relies on reliable spotted mask detection. Teammates only show as visible when the game actually marks them spotted. Snappier overall feel and consistent head circle sizing.
  - **Improved radar & bomb timer** — Fixed ghost dropped bombs that could linger after pickup. Better bomb state tracking and timing data. Reorganized bomb-related toggles into the Radar page for clarity.
  - **Per-monitor DPI awareness** — Menus and overlays now scale correctly and stay crisp on any Windows display scaling setting.
  - **New Sounds category** — Dedicated section in the Extras page for managing hit and kill sounds.
  - **Team check respect in aiming** — Aim assist now properly respects your team check setting.

  ### Bug fixes

  - Major stability improvements from removing unstable visibility code and dead features.
